...
def trainer_synapse(args, model, snapshot_path, multimask_output, low_res):
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            '''revise'''
            # print(image_batch.shape) torch.Size([12, 3, 512, 512])
            ...
            # points_coords = torch.floor(torch.Tensor(points_coords)).int().cuda()
            # inputs_batch=[]
            ...
            #     inputs_batch.append({'image':image_batch[i],'original_size':(args.img_size,args.img_size),'point_coords':points_coords,'point_labels':point_labels})
            ...
            # print(outputs['low_res_logits'].shape) torch.Size([12, 14, 128, 128])
            finish revise
            '''
            outputs = model(image_batch, multimask_output, args.img_size)         
