...
def test_single_volume(image, label, net, classes, multimask_output, patch_size=[256, 256], input_size=[224, 224],
    ...
    if len(image.shape) == 3:
        ...
        for ind in range(image.shape[0]):
            ...
            # x_point = np.linspace(0, imgsize-1, num_points)
            # y_point = np.linspace(0, imgsize-1, num_points)
        ...
        for i in range(label.shape[0]):
            # imageio.imwrite(f'/data/nanxi/code/SAMed/nxoutput/images/pred/pred_{i}.png', prediction[i])
            # imageio.imwrite(f'/data/nanxi/code/SAMed/nxoutput/images/label/label_{i}.png', label[i])
